...
- PRJ-02 completed a static read-only implementation/evidence/privacy-boundary audit of CourtFit at repository `https://github.com/nattapong18-en/chat-bot-test`, branch `main`, commit `3c9cc65d00622731b4a381ddb785edc3713f8c46`; `reference/audits/PRJ-02-courtfit-evidence.md` governs commit-scoped source claims only.
...
**Requires verification:** Public identity details and contact methods where not already confirmed; Booking API build, migration, test execution, runtime routes, cross-user isolation, concurrency integrity, Redis correction/runtime behavior, Docker operation, Render/Vercel status, historical environment-file risk, and artifact publication readiness; CourtFit build/lint/format/type-check execution, successful OpenAI/Gemini calls and current compatibility, operational key/log/HTTPS/provider-retention behavior, whether raw server-mediated BYOK should remain, incremental-forwarding direction, Stop/Retry runtime behavior, mobile action correction, browser/responsive/accessibility behavior, Markdown/product formatting, verified shoe data, recommendation quality, automated tests, artifacts/screenshots, deployment/source equivalence, commit-author-email privacy, licensing, and publication approval; publishable artifacts; coursework and learning evidence; research sources and available hardware. Booking API source findings are resolved only for the PRJ-01 audited commit. CourtFit architecture, browser → CourtFit server → provider key flow, full-history behavior, buffered provider-stream handling, source-level controls, and absent catalog/test/deployment evidence are resolved only for the PRJ-02 audited commit.
...
**Main work:** Verify personal profile details, public name presentation, and contact readiness. Use the committed PRJ-01 record for Booking API static facts rather than re-accessing its repository without new authorization; preserve its runtime, historical-secret, redaction, and publication blockers. Use the committed PRJ-02 record for CourtFit static facts rather than re-accessing its repository; preserve its runtime/provider/privacy/mobile/accessibility/testing/artifact/deployment/publication blockers. Treat CourtFit source-tree, route, flow-diagram, adapter, buffering, Stop/Retry, responsive, preview-state, and history-summary material only as conditional artifact candidates requiring accuracy, redaction, privacy, rights, and publication review; do not assume screenshots, logs, builds, tests, or deployments exist. Record status, maturity, role, contribution, limitations, certainty, verification, and last-reviewed context. Prepare Thai, English, and mixed-language samples. Prepare Research Preparation content without selecting a graduation topic. Curate a small number of learning and coursework examples. Complete redaction and privacy review and identify content gaps.
...
Use the PRJ-02 evidence record for audited commit `3c9cc65d00622731b4a381ddb785edc3713f8c46` only. Plan CourtFit as an individual project according to Nattapong's confirmed internal record, an educational and experimental basketball-shoe recommendation chatbot, a domain-specific conversational prototype, and a source-level functional-MVP candidate with full-stack/AI integration source. Do not present it as a real store or production-ready.

The source overview may cover Next.js App Router, React, TypeScript, Tailwind CSS, local shadcn-style/Radix components, one page route (`/`), and one dynamic Node API route (`POST /api/chat`). Preserve the boundary between browser React state, the CourtFit application server, and server-only OpenAI/Google GenAI adapters; do not introduce planning-only routes or product endpoints.

Explain the user-key flow exactly as browser React state → CourtFit server → selected provider. Every request and Retry sends the key through the server to a request-scoped provider client; the browser does not send it directly to the provider. No direct project persistence or project key logging was found, but deployment infrastructure, HTTPS, middleware, memory, browser scripts/extensions, logs/observability, operational controls, provider retention, and privacy security remain unverified. Chat history is React-memory-only in inspected source, and all current non-empty retained user/assistant messages pass through the server each turn; provider/key changes clear conversation and key state.

Describe response handling as: “Provider streams are consumed server-side, but the audited route buffers the output and sends one assembled response event to the client.” The client NDJSON reader does not establish incremental assistant display. Stop/Retry and abort paths exist in source but require runtime verification for provider cancellation, races, partial preservation, billing effects, success, and duplicate prevention. OpenAI/Gemini switching clears incompatible state; no model picker exists, desktop actions are wired, and mobile drawer New Chat/provider/key action parity is incomplete.

Keep the product and disclosure boundary visible: no structured shoe catalog, database/API, product images/cards/details, verified price/stock feed, cart, checkout, or commerce transaction exists. Recommendations are model-generated, accuracy-unverified, and may contain inaccurate or fabricated product information. Rendered guidance covers simulated advice, unavailable live price/stock, and key/provider-charge responsibility only; explicit educational-use/not-official-store and full AI-inaccuracy wording remain missing.

Responsive/mobile classes, a Radix drawer, adaptive sizing/overflow, dynamic viewport height, light/dark/system themes, labels, focus/status/live-region patterns, IME handling, and some reduced-motion handling are static source evidence only. No browser, physical-device, visual, keyboard, screen-reader, contrast, zoom/reflow, touch, orientation, virtual-keyboard, or accessibility testing occurred. Messages are plain pre-wrapped text without Markdown rendering.

No automated test source, runner/framework, test script, coverage configuration, or CI workflow was found. Build, lint, type-check, and format configuration exists but was not run; preview states and `data-testid` attributes are not tests, and Chrome/Firefox/Safari testing is self-reported and undocumented. Vercel is documentation context only; no live URL, Vercel configuration, deployment workflow, or result was verified. Require commit-author-email privacy review, artifact/redaction/rights review, deployment/source-equivalence verification, licensing review, and publication approval before public use.
...
**Validation and exit:** No secret or unapproved private data is exposed; CourtFit key claims preserve browser React state → CourtFit server → selected provider and full-history transmission, plus all operational unknowns; optional failure preserves content; security and privacy claims are supported rather than assumed.
...
- **Privacy leakage and API-key misunderstanding:** Prevent through redaction and evidence-bounded review. Preserve the audited CourtFit browser → application server → selected provider key/history boundary and its unresolved operational logging, transport, memory, browser, and provider-retention risks; preserve the unresolved private review of the Booking API's historical `.env` path. Detect through separately authorized repository/runtime/network/media review; respond by removing exposure, rotating potentially affected credentials when appropriate, correcting claims, and reassessing release.
...
- Is the PRJ-02 audited CourtFit commit still canonical, and what separate evidence can verify build/checks, provider/model compatibility and successful calls, operational key/log/HTTPS/provider-retention behavior, raw server-mediated BYOK direction, incremental forwarding, Stop/Retry runtime behavior, mobile parity, browser/responsive/accessibility behavior, Markdown/product formatting, verified shoe data and recommendation quality, automated tests, artifacts, deployment/source equivalence, author-email privacy, licensing, and publication readiness?
